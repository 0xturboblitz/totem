...
template ShortSignedEmailRegex(msg_bytes) {
	...
	for (var i = 0; i < msg_bytes; i++) {
		is_substr0[i][0] <== 0;
		is_substr0[i][1] <== is_substr0[i][0] + states[i+1][3] * states[i+2][6];
		is_substr0[i][2] <== is_substr0[i][1] + states[i+1][6] * states[i+2][6];
